...
public class SpeechManager : MonoBehaviour {
    ...
    private void PlayAudio(Stream audioStream, string title)
    {
    ...
    /// <returns></returns>
    private IEnumerator WaitAndPlayRoutineREST(Task<Stream> speakTask, string title)
    {
        // Yield control back to the main thread as long as the task is still running
    ...
    private void RegisterClip(string title, MemoryStream resultStream)
    {
        ...
        //ByteArrayToFile("Assets/Resources/" + title + ".wav", buffer);
    ...
    public void SpeakWithRESTAPI(string message)
    {
        try
        {
            ...
            StartCoroutine(WaitAndPlayRoutineREST(Speaking, message));
        }
        catch (Exception ex)
        {
